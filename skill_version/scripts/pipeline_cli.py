...
def build_mappings(config: dict) -> dict:
    """
    为每张表构建字段映射。
    ...
    for table, file_ref in config["files"].items():
        # 读取预览 (只需前几行)
        ...
        final_map = {}
        schema_req = TABLE_SCHEMA[table]['required']

        for std_col in schema_req:
            # 优先用覆盖
...
def _build_index(ctx) -> dict:
    ...
    for (y, m), calc in ctx.monthly_calc.items():
        ...
        month_summaries[key] = {
            ...
            "computation_time_seconds": round(ctx.log.stage_times.get('S4_核心矩阵运算', 0), 3) if (y, m) == ctx.all_months[0] else None,
...
def main():
    ...
    try:
        ...
        mapping_dict = build_mappings(config)
