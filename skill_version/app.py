    ...
    if st.button("🚀 执行成本核算", type="primary", use_container_width=True):
        with st.spinner("正在读取并聚合数据..."):
            try:
                ...
                # 显示流水线日志
                with st.expander("📋 流水线运行日志", expanded=ctx.log.has_errors):
                    st.code(ctx.log.summary(), language=None)

                if ctx.log.has_errors:
                    st.error(f"❌ 流水线中断：{len(ctx.log.errors)} 个错误")
                    return
                ...
                # 将 pipeline 结果映射到 session_state
                st.session_state.all_months = ctx.all_months
                st.session_state.monthly_results = ctx.monthly_results
                st.session_state.monthly_calc = ctx.monthly_calc
                st.session_state.monthly_perf = {
                    (y, m): c.get_performance()
                    for (y, m), c in ctx.monthly_calc.items()
                }
                st.session_state.monthly_total_time = {
                    (y, m): ctx.log.stage_times.get('S4_核心矩阵运算', 0)
                    for (y, m) in ctx.all_months
                }

                st.session_state.calculation_done = True
                st.success(f"✅ SOP 流水线完成：{len(ctx.all_months)} 个月核算 | "
                          f"S1→S2→S3→S4→S5")
